...
func (obj *findHandlerImpl) OnFindResult(browser Browser, identifier int32, count int32, selectionrect *Rect, activematchordinal int32, finalupdate int32) {
	...
	rawPtr := obj.rawPtr
	rawPtr.CallOnFindResult(extractRawPointer(browser), uintptr(identifier), uintptr(count), unsafe.Pointer(selectionrect), uintptr(activematchordinal), uintptr(finalupdate))
}
