...
- `cef.Settings` instead of `cef.CEFSettings`
- `cef.NewClient(...)` with `cef.SafeClient`
- `cef.NewLifeSpanHandler(...)` with `cef.SafeLifeSpanHandler`
- `cef.NewAudioHandler(...)` with the decoded `cef.AudioHandler` interface

Treat `CEF*` structs and the low-level raw handler interfaces as advanced escape hatches.
...
// Implement the safe client interface — only the methods you need.
type myClient struct{}

func (c myClient) GetLifeSpanHandler() cef.SafeLifeSpanHandler { return nil }
