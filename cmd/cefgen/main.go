...
func writeRawRegisterAggregator(dir string, names []string) error {
	...
	sb.WriteString("}\n")
	if err := os.WriteFile(filepath.Join(dir, "register.go"), []byte(sb.String()), 0o644); err != nil {
		return err
	}
...
func writePlatformRegisterFile(dir, goos string, names []string) error {
	...
	sb.WriteString("}\n")
	return os.WriteFile(filepath.Join(dir, "register_platform_"+goos+".go"), []byte(sb.String()), 0o644)
}
...
func writePlatformRegisterDefault(dir string) error {
	...
	return os.WriteFile(filepath.Join(dir, "register_platform_default.go"), []byte(sb.String()), 0o644)
}
